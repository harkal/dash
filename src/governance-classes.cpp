...
CAmount CSuperblock::GetPaymentsLimit(int nBlockHeight)
{
    ...
    int nBits = consensusParams.fPowAllowMinDifficultyBlocks ? consensusParams.powLimit.GetCompact() : 1;
...
void CSuperblock::ParsePaymentSchedule(std::string& strPaymentAddresses, std::string& strPaymentAmounts)
{
    ...
    */
    ...
    for (int i = 0; i < (int)vecParsed1.size(); i++) {
        ...
        if (!address.IsValid()) {
            std::ostringstream ostr;
            ostr << "CSuperblock::ParsePaymentSchedule -- Invalid Ebakus Address : " <<  vecParsed1[i];
            LogPrintf("%s\n", ostr.str());
            throw std::runtime_error(ostr.str());
        }
