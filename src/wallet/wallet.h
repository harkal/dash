...
private:
  /** Constant used in hashBlock to indicate tx has been abandoned */
    static const H256 ABANDON_HASH;
...
public:
    ...
    template <typename Stream, typename Operation>
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        std::vector<H256> vMerkleBranch; // For compatibility with older versions.
...
public:
    ...
    std::set<H256> GetConflicts() const;
};
...
private:
     ...
     */
    typedef std::multimap<COutPoint, H256> TxSpends;
    TxSpends mapTxSpends;
    void AddToSpends(const COutPoint& outpoint, const H256& wtxid);
    void AddToSpends(const H256& wtxid);

    /* Mark a transaction (and its in-wallet descendants) as conflicting with a particular block. */
    void MarkConflicted(const H256& hashBlock, const H256& hashTx);
...
public:
    ...
    std::map<H256, CWalletTx> mapWallet;
    ...
    std::map<H256, int> mapRequestCount;
    ...
    const CWalletTx* GetWalletTx(const H256& hash) const;
    ...
    /// Get 1000EBAKUS output and keys which can be used for the Masternode
    ...
    bool IsSpent(const H256& hash, unsigned int n) const;

    bool IsLockedCoin(H256 hash, unsigned int n) const;
    ...
    std::vector<H256> ResendWalletTransactionsBefore(int64_t nTime, CConnman* connman);
    ...
    bool GetBudgetSystemCollateralTX(CTransaction& tx, H256 hash, CAmount amount, bool fUseInstantSend);
    bool GetBudgetSystemCollateralTX(CWalletTx& tx, H256 hash, CAmount amount, bool fUseInstantSend);
    ...
    bool UpdatedTransaction(const H256 &hashTx);

    void Inventory(const H256 &hash)
    {
        {
            LOCK(cs_wallet);
            std::map<H256, int>::iterator mi = mapRequestCount.find(hash);
    ...
    std::set<H256> GetConflicts(const H256& txid) const;
     ...
     * @note called with lock cs_wallet held.
     */
    boost::signals2::signal<void (CWallet *wallet, const H256 &hashTx,
    ...
    bool AbandonTransaction(const H256& hashTx);
