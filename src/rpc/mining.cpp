...
UniValue generate(const UniValue& params, bool fHelp)
{
    ...
    {
        std::unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlock(Params(), minerPubKey));
...
UniValue prioritisetransaction(const UniValue& params, bool fHelp)
{
    ...
    LOCK(cs_main);
    H256 hash = ParseHashStr(params[0].get_str(), "txid");
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Ebakus Core is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Ebakus Core is downloading blocks...");

    if (!masternodeSync.IsSynced())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Ebakus Core is syncing with network...");
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5))
    {
        ...
        pblocktemplate = CreateNewBlock(Params(), CPubKey());
        if (!pblocktemplate)
...
public:
    ...
    submitblock_StateCatcher(const H256 &hashIn) : hash(hashIn), found(false), state() {};
