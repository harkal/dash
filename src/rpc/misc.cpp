...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total ebakus balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized ebakus balance of the wallet\n"
...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "ebakus|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use comma to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "ebakus")
            + HelpExampleRpc("debug", "ebakus,net")
        );
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"ebakusaddress\"\n"
            "\nReturn information about the given ebakus address.\n"
            "\nArguments:\n"
            "1. \"ebakusaddress\"     (string, required) The ebakus address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"ebakusaddress\", (string) The ebakus address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Ebakus address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are ebakus addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) ebakus address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"ebakusaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"ebakusaddress\"  (string, required) The ebakus address to use for the signature.\n"
...
bool getAddressFromIndex(const int &type, const H160 &hash, std::string &address)
{
    if (type == 2) {
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<H160, int> > &addresses)
{
    if (params[0].isStr()) {
        CBitcoinAddress address(params[0].get_str());
        uint160 hashBytes;
...
UniValue getaddressmempool(const UniValue& params, bool fHelp)
{
    ...
    std::vector<std::pair<H160, int> > addresses;
