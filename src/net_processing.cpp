...
map<H256, COrphanTx> mapOrphanTransactions GUARDED_BY(cs_main);
map<H256, set<H256> > mapOrphanTransactionsByPrev GUARDED_BY(cs_main);;
...
namespace {
     ...
     * cs_main.
     */
    map<H256, NodeId> mapBlockSource;
    ...
    map<H256, pair<NodeId, list<QueuedBlock>::iterator> > mapBlocksInFlight;
...
bool MarkBlockAsReceived(const H256& hash) {
    map<H256, pair<NodeId, list<QueuedBlock>::iterator> >::iterator itInFlight = mapBlocksInFlight.find(hash);
...
// Requires cs_main.
void MarkBlockAsInFlight(NodeId nodeid, const H256& hash, const Consensus::Params& consensusParams, CBlockIndex *pindex = NULL) {
    CNodeState *state = State(nodeid);
    assert(state != NULL);
...
void UpdateBlockAvailability(NodeId nodeid, const H256 &hash) {
    CNodeState *state = State(nodeid);
    assert(state != NULL);
...
void static EraseOrphanTx(H256 hash) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...
{
    int nErased = 0;
    map<H256, COrphanTx>::iterator iter = mapOrphanTransactions.begin();
    while (iter != mapOrphanTransactions.end())
    {
        map<H256, COrphanTx>::iterator maybeErase = iter++; // increment to avoid iterator becoming invalid
...
unsigned int LimitOrphanTxSize(unsigned int nMaxOrphans) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    while (mapOrphanTransactions.size() > nMaxOrphans)
    {
        ...
        H256 randomhash = GetRandHash();
        map<H256, COrphanTx>::iterator it = mapOrphanTransactions.lower_bound(randomhash);
        if (it == mapOrphanTransactions.end())
    ...
    if (!fInitialDownload) {
        ...
        connman->ForEachNode([nNewHeight, &vHashes](CNode* pnode) {
            if (nNewHeight > (pnode->nStartingHeight != -1 ? pnode->nStartingHeight - 2000 : 0)) {
                BOOST_REVERSE_FOREACH(const H256& hash, vHashes) {
...
void PeerLogicValidation::BlockChecked(const CBlock& block, const CValidationState& state) {
    ...
    const H256 hash(block.GetHash());
    std::map<H256, NodeId>::iterator it = mapBlockSource.find(hash);
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Ebakus Related Inventory Messages
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::TX || strCommand == NetMsgType::DSTX || strCommand == NetMsgType::TXLOCKREQUEST)
    {
        ...
        vector<H256> vWorkQueue;
        vector<H256> vEraseQueue;
        CTransaction tx;
        CTxLockRequest txLockRequest;
        ...
        if (!AlreadyHave(inv) && AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs))
        {
            ...
            {
                map<H256, set<H256> >::iterator itByPrev = mapOrphanTransactionsByPrev.find(vWorkQueue[i]);
                if (itByPrev == mapOrphanTransactionsByPrev.end())
                    continue;
                for (set<H256>::iterator mi = itByPrev->second.begin();
                     mi != itByPrev->second.end();
                     ++mi)
                {
                    const H256& orphanHash = *mi;
            ...
            BOOST_FOREACH(H256 hash, vEraseQueue)
                EraseOrphanTx(hash);
        }
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            connman.PushMessage(pfrom, NetMsgType::GETHEADERS, chainActive.GetLocator(pindexLast), H256());
        }
    ...
    else if (strCommand == NetMsgType::MEMPOOL)
    {
        ...
        std::vector<H256> vtxid;
        mempool.queryHashes(vtxid);
        vector<CInv> vInv;
        BOOST_FOREACH(H256& hash, vtxid) {
...
bool ProcessMessages(CNode* pfrom, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (!pfrom->vRecvGetData.empty()) return true;
        ...
        H256 hash = Hash(vRecv.begin(), vRecv.begin() + nMessageSize);
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 6 * 60 * 60) { // NOTE: was "close to today" and 24h in Bitcoin
                ...
                connman.PushMessage(pto, NetMsgType::GETHEADERS, chainActive.GetLocator(pindexStart), H256());
            }
        }
        ...
        //
        {
            ...
            if (!fRevertToInv) {
                ...
                BOOST_FOREACH(const H256 &hash, pto->vBlockHashesToAnnounce) {
            ...
            if (fRevertToInv) {
                ...
                if (!pto->vBlockHashesToAnnounce.empty()) {
                    const H256 &hashToAnnounce = pto->vBlockHashesToAnnounce.back();
