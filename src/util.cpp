...
const char * const BITCOIN_CONF_FILENAME = "ebakus.conf";
const char * const BITCOIN_PID_FILENAME = "ebakusd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "ebakus" is a composite category enabling all Ebakus-related debug output
            if(ptrCategory->count(string("ebakus"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EbakusCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EbakusCore
    // Mac: ~/Library/Application Support/EbakusCore
    // Unix: ~/.ebakuscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EbakusCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EbakusCore";
#else
    // Unix
    return pathRet / ".ebakuscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ebakus.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ebakus.conf
