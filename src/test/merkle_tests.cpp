...
static H256 BlockBuildMerkleTree(const CBlock& block, bool* fMutated, std::vector<H256>& vMerkleTree)
{
    vMerkleTree.clear();
    ...
    return (vMerkleTree.empty() ? H256() : vMerkleTree.back());
}

// Older version of the merkle branch computation code, for comparison.
static std::vector<H256> BlockGetMerkleBranch(const CBlock& block, const std::vector<H256>& vMerkleTree, int nIndex)
{
    std::vector<H256> vMerkleBranch;
    int j = 0;
    for (int nSize = block.vtx.size(); nSize > 1; nSize = (nSize + 1) / 2)
    {
...
BOOST_AUTO_TEST_CASE(merkle_test)
{
    for (int i = 0; i < 32; i++) {
        ...
        for (int mutate = 0; mutate <= 3; mutate++) {
            ...
            H256 unmutatedRoot = BlockMerkleRoot(block, &unmutatedMutated);
            ...
            std::vector<H256> merkleTree;
            H256 oldRoot = BlockBuildMerkleTree(block, &oldMutated, merkleTree);
            // Compute the merkle root using the new mechanism.
            bool newMutated = false;
            H256 newRoot = BlockMerkleRoot(block, &newMutated);
            BOOST_CHECK(oldRoot == newRoot);
            BOOST_CHECK(newRoot == unmutatedRoot);
            BOOST_CHECK((newRoot == H256()) == (ntx == 0));
            ...
            if (mutate == 0) {
                for (int loop = 0; loop < std::min(ntx, 16); loop++) {
                    ...
                    std::vector<H256> newBranch = BlockMerkleBranch(block, mtx);
                    std::vector<H256> oldBranch = BlockGetMerkleBranch(block, merkleTree, mtx);
