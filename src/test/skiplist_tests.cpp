...
BOOST_AUTO_TEST_CASE(getlocator_test)
{
    ...
    std::vector<H256> vHashMain(100000);
    ...
    std::vector<H256> vHashSide(50000);
    std::vector<CBlockIndex> vBlocksSide(50000);
    for (unsigned int i=0; i<vBlocksSide.size(); i++) {
        vHashSide[i] = H256(i + 50000 + (U256(1) << 128)); // Add 1<<128 to the hashes, so GetLow64() still returns the height.
