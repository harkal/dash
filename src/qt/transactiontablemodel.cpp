...
struct TxLessThan
{
    ...
    bool operator()(const TransactionRecord &a, const H256 &b) const
    {
        return a.hash < b;
    }
    bool operator()(const H256 &a, const TransactionRecord &b) const
    {
        return a < b.hash;
    }
};
...
public:
    ...
    void refreshWallet()
    {
        ...
        cachedWallet.clear();
        {
            ...
            for(std::map<H256, CWalletTx>::iterator it = wallet->mapWallet.begin(); it != wallet->mapWallet.end(); ++it)
            {
     ...
     */
    void updateWallet(const H256 &hash, int status, bool showTransaction)
    {
        ...
        case CT_NEW:
            ...
            if(showTransaction)
            {
                ...
                std::map<H256, CWalletTx>::iterator mi = wallet->mapWallet.find(hash);
    ...
    TransactionRecord *index(int idx)
    {
        if(idx >= 0 && idx < cachedWallet.size())
        {
            ...
            if(lockMain)
            {
                ...
                {
                    std::map<H256, CWalletTx>::iterator mi = wallet->mapWallet.find(rec->hash);
...
public:
    ...
    TransactionNotification(H256 hash, ChangeType status, bool showTransaction):
...
static void NotifyTransactionChanged(TransactionTableModel *ttm, CWallet *wallet, const H256 &hash, ChangeType status)
{
    // Find transaction in wallet
    std::map<H256, CWalletTx>::iterator mi = wallet->mapWallet.find(hash);
