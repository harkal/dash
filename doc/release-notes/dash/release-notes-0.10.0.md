...
- Darksend tested with 2.5 EBK successfully and I've set that as the minimum
...
- Fixed the endless splitting issue causes by splitting 1000EBK and not making a DS compatible input
...
- Fixed issue with denominating small amounts of EBK in large wallet
  (http://jira.darkcoin.qa/browse/DRK-46)
- Made splitting up initial inputs much more efficient. Now when splitting up,
  it will use powers of two from 4096 EBK in reverse to get the best possible
...
- Darksend limited to 5000EBK per wallet.dat. Client will warn about this the
  first time it's opened, then disable darksend from then on.
- Fixed some bugs with connecting to the correct masternodes
- Send was sending way too many coins for all modes, (I sent 100EBK anon and it
  sent 2000EBK, then sent me change for the rest causing a whole reprocess of
...
- Fees per round of Darksend are 0.001EBK or $0.00538 at current prices. This
  means to anonymize 1000EBK with 3 rounds (an average use case) it would cost
  a user 1.5 cents.
...
- RPC calls are changed a bit:
    ...
    darksend denominate 100 - Will denominate 100EBK
    darksend auto - Will run AutoDenominate
    darksend Xaddr 100 - Will send 100 denominated EBK to Xaddr
    sendtoaddress Xaddr 100 - Will send 100 non-denominated EBK to Xaddr
...
- Added a smart input splitting method. Place 1000+EBK into a brand new wallet
...
- Mixing rounds are limited to 1000EBK, per block
...
- DoAutomaticDenominations 100EBK limit changed to 500EBK (we should see a bunch
