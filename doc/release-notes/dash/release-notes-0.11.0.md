...
- Defined BIP32 (HD) address versions to start with `drkp`/`drkv` (`EBKP`/`EBKV`
  for testnet)
- Adapted BIP44 coin type `5` for Darkcoin (0x80000005) as defined in SLIP-0044
- Added new units: `duffs` (1 / 100.000.000 EBK)
- Added units for testnet: tEBK, mtEBK, utEBK, tduffs
...
- Reduced lower darksend limit to 1.5 EBK
- Fixed progress bar calculation for low amounts
- Improved support for adding EBK after anon has completed
...
- Add '-disablewallet' mode to let ebakusd run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with ebakusd
...
- Drop the fee required to relay a transaction to 0.01mEBK per kilobyte
