...
*Setup instructions for a Gitian build of Ebakus Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Ebakus
...
- [Building Ebakus Core](#building-ebakus-core)
...
Clone the git repositories for Ebakus Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ebakuspay/ebakus
```
...
in the Ebakus Core repository under 'Fetch and build inputs' to install sources which require
...
To build Ebakus Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Ebakus Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ebakus/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ebakuspay/ebakus
...
For example:
```bash
URL=https://github.com/crowning-/ebakus.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit ebakus=${COMMIT} --url ebakus=${URL} ../ebakus/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ebakus=${COMMIT} --url ebakus=${URL} ../ebakus/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ebakus=${COMMIT} --url ebakus=${URL} ../ebakus/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the ebakus git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ebakus/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/ebakus.git
SIGPATH=/some/root/path/ebakus-detached-sigs.git

./bin/gbuild --url ebakus=${BTCPATH},signature=${SIGPATH} ../ebakus/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ebakus-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ebakus-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ebakus-osx-build.assert
```
...
[ebakus/gitian.sigs](https://github.com/ebakuspay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
